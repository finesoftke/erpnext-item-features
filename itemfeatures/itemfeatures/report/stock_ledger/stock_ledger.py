...
def get_columns(filters):
	columns = [
		...
		{
			"label": _("Feature"),
			"fieldname": "custom_feature",
			"fieldtype": "Link",
			"options": "Item Feature",
			"width": 100,
		},
		{
			"label": _("Stock UOM"),
...
def get_stock_ledger_entries(filters, items):
	...
	if filters.get("custom_feature"):
		query = query.where((sle.custom_feature == filters.get("custom_feature")))
...
def get_opening_balance(filters, columns, sl_entries):
	if not (filters.item_code and filters.warehouse and filters.from_date and filters.custom_feature):
		return
	...
	last_entry = get_previous_sle(
		{
			...
			"custom_feature": filters.custom_feature
		}
	)
